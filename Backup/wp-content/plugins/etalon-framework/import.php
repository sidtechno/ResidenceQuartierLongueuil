...
if(isset($_POST['url'])) {
	...
	if( isset($keydesign_check_files['erros']) ){
		...
		$keydesign_blog_page = get_page_by_title( 'Blog' );
		update_option( 'page_for_posts', $keydesign_blog_page->ID );

		// Import theme options JSON file

		$json_file = file_get_contents($_POST['url'] . '/theme_options.json');
	    update_option( 'redux_ThemeTek', json_decode($json_file, true), '', 'yes' );
	

		// Set up the theme menus
		// Set up theme main menu
		$keydesign_main_menu_name = wp_get_nav_menu_object( 'Main Menu' );
		$keydesign_term_id_of_main_menu = $keydesign_main_menu_name->term_id;
		$keydesign_main_menu_location = get_theme_mod('nav_menu_locations');
		$keydesign_main_menu_location['keydesign-header-menu'] = $keydesign_term_id_of_main_menu;
		set_theme_mod( 'nav_menu_locations', $keydesign_main_menu_location );

		// Set up theme footer menu
		$keydesign_footer_menu_name = wp_get_nav_menu_object( 'Footer menu' );
		$keydesign_term_id_of_footer_menu = $keydesign_footer_menu_name->term_id;
		$keydesign_footer_menu_location = get_theme_mod('nav_menu_locations');
		$keydesign_footer_menu_location['keydesign-footer-menu'] = $keydesign_term_id_of_footer_menu;
		set_theme_mod( 'nav_menu_locations', $keydesign_footer_menu_location );
