...
if (!class_exists('tek_masonrygallery_elem')) {
    class tek_masonrygallery_elem extends KEYDESIGN_ADDON_CLASS {
                ...
                vc_map(array(
                    ...
                    "params" => array(
                        ...
                        array(
                            "type" =>	"dropdown",
                            "class" => "",
                            "heading" => esc_html__("Grid size","keydesign"),
                            "param_name" => "mg_size",
                            "value" => array(
                                    "Small" => "mg-small",
                                    "Big" => "mg-big",
        ...
        public function kd_masonrygallery_single($atts, $content = null) {
            ...
            // Declare empty vars
            $output = $image = $mg_single_size = $link = '';
            ...
            if ($mg_image != '') {
                $output = '<div class="mg-single-img '.$mg_single_size.'"><a data-size="' . $width. 'x' .$height .'" href='. $link . '>' . $image['thumbnail'] . '</a></div>';
            } else {
                $output = '<div class="mg-single-img '.$mg_single_size.'">' . $default_src . '</div>';
            }
            return $output;
        }
    }
